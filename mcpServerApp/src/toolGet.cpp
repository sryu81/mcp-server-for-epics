...
void toolGetSchema(yajl_gen gen)
{
    ...
    mcpGenKeyString(gen, "description", "PV name(s) to read");
    yajl_gen_map_close(gen);
...
int toolGetHandler(const McpJsonValue *params, yajl_gen gen)
{
    ...
    if (!pvnamesVal || pvnamesVal->type != MCP_JSON_ARRAY || mcpJsonArrayCount(pvnamesVal) == 0) {
        yajl_gen_map_open(gen);
    ...
    McpJsonValue *tv = params ? mcpJsonMapGet(params, "timeout") : NULL;
    ...
    if (strcmp(protocol, "ca") == 0) {
        ...
        result = caOps::get(pvnames, timeout);
    } else {
        ...
        result = pvaOps::get(pvnames, timeout);
    }
